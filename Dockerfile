...
WORKDIR /home/jovyan/work/covid-stats
...
RUN pip install --no-cache-dir xlrd==1.2.* requests==2.25.*

# Copy project files from the builder
USER ${PY_USER}
WORKDIR /home/${PY_USER}/work/covid-stats
COPY --from=builder --chown=jovyan:jovyan /home/jovyan/work/covid-stats/ ./
RUN mkdir data docs
